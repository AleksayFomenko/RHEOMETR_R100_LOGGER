...
def _find_arial_ttf() -> str:
    """
    Ищет Arial.ttf в стандартных местах для текущей ОС.
    Порядок: рядом с exe/скриптом → системные шрифты Windows → системные macOS.
    Выбрасывает FileNotFoundError, если шрифт не найден нигде.
    """
    # 1. Рядом с исполняемым файлом (актуально для PyInstaller bundle)
    ...
    candidates = [
        ...
        os.path.join(base, "..", "..", "Arial.ttf"),  # корень проекта
        "Arial.ttf",                                   # текущая рабочая директория
    ]
    ...
    # 3. Системные шрифты macOS (для разработки на маке)
    ...
    raise FileNotFoundError(
        "Шрифт Arial.ttf не найден.\n"
        "Скопируйте Arial.ttf в папку с программой или установите шрифты Microsoft TrueType."
    )
...
def export_pdf(
    ...
    elements.append(Paragraph("Протокол испытаний ГОСТ Р 54547-2011", styles["Title"]))
